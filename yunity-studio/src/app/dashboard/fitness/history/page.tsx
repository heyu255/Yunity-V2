...
function computeStreak(history: { created_at: string }[]) {
...
function computeWeekVolume(history: { created_at: string; exercise_logs: any[] }[], weeksAgo = 0) {
  const now = new Date()
...
function topMuscleGroup(history: { dayFocus: string }[]) {
...
function daysSinceLastWorkout(history: { created_at: string }[]) {
